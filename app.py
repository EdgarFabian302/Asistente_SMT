...
st.header("1. Condición Ambiental y de Proceso (GKG P6 Max)")
col1, col2 = st.columns(2)

with col1:
    temperatura = st.slider("Temperatura del Cuarto (°C)", 18.0, 35.0, 23.5)
    presion = st.slider("Presión de Rasqueta (Kg)", 3.0, 15.0, 8.5)
...
if archivo_cargado is not None:
    # Usar nuestro módulo spi_parser para procesar la información
    df_spi = cargar_reporte_spi(archivo_cargado)
    
    if df_spi is not None:
        st.success("¡Archivo cargado y procesado correctamente!")
        
        tab1, tab2 = st.tabs(["📋 Datos Completos", "⚠️ Defectos Detectados"])
        ...
        with tab2:
            ...
            if not defectos.empty:
                ...
                st.success("No se encontraron defectos en esta tablilla.")